...
function responseFor(request) {
  ...
  if (request.method === 'tools/list') {
    return { result: { tools: [
      { name: 'list_pages', description: 'Lists pages.', inputSchema: { type: 'object', properties: { domain: { type: 'string' } }, required: ['domain'] } },
      { name: 'get_page', description: 'Gets a page.', inputSchema: { type: 'object', properties: { url: { type: 'string' } }, required: ['url'] } }
    ] } };
  }
...
test('createHandler composes configured remote tools', async (t) => {
  ...
  const handler = await createHandler({ tools: [], remote: [{ name: 'probeo', url: `http://127.0.0.1:${address.port}`, tools: ['list_pages'] }] });

  const listed = await handler({ jsonrpc: '2.0', id: 1, method: 'tools/list' });
  assert.deepEqual(listed.result.tools.map((tool) => tool.name), ['probeo.list_pages']);
