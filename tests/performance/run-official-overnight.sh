#!/usr/bin/env bash
# Overnight sustained mixed-workload runner for all 10 official MCP SDKs.
# Runs every (duration × language) combination, saves raw JSON + log per run.
#
# Order: by-duration (outer loop), so the 1m matrix completes first.
# Total wall time: ~3.5 hours (1m+3m+5m+10m × 10 langs + container overhead).
#
# Usage:  bash tests/performance/run-official-overnight.sh
# Output: zeromcp-benchmarks/results/mixed-workload-official-overnight/
...
OUT="/Users/chriswelker/Projects/antidrift/zeromcp-benchmarks/results/mixed-workload-official-overnight"
...
run_one() {
  ...
  if ! docker run -d --name "$CONTAINER" -p "${PORT}:3000" \
        "zeromcp-bench-official-http-${lang}" >/dev/null 2>&1; then
