...
**Shipped**, on branch `registry/ruby`. Implemented exactly as designed below: `ZeroMcp::Registry.create`/`ZeroMcp.create_registry` in a new `registry.rb`, `RouteDefinition`/`ToolRegistry` structs (`ToolRegistry#context_for` included), plus the two supporting extractions — `openapi.rb` (`OpenApi.build`, moved verbatim from `Server#build_openapi_spec` + its three private helpers) and `credentials.rb` (`Credentials.resolve`, moved verbatim from `Server#_resolve_credentials` + its two private helpers). `Server#initialize` gained the backward-compatible `tools:` kwarg; `Tool#route_method`/`#route_path` were added and adopted by both `Server#serve_http`'s route-registration loop and `OpenApi.build`. One small addition beyond the spec's literal sketch: `Registry.create` takes `title:`/`execute_timeout:` keyword args (per "Porting notes"), threaded into `Config.new` when no explicit `config:` is given — `version:` was deliberately **not** added, since `OpenApi.build` hardcodes `'0.5.0'` regardless (a separate, out-of-scope issue per the "Fixed" section below) and a `version:` kwarg with no consumer would be dead API surface.

Covered by 10 new tests in `test/test_registry.rb` (route filtering, insertion-order preservation, OpenAPI parity, direct tool invocation via `context_for`, `mcp` dispatch for `tools/list` and `tools/call`, empty-routes case, the `Config.new`-not-`Config.load` default, and `title:`/`execute_timeout:` threading). Full existing suite (112 runs across all 9 test files, including `test_server.rb`'s 26/26) passes with zero regressions — `build_openapi_spec`, credential resolution, and route registration all still work correctly after being refactored into thin delegates. Also manually booted `Server#serve_http` and confirmed `/openapi.json` output is byte-identical to before the extraction for the `echo`/`greet` conformance fixtures.

One minor, non-blocking inefficiency worth knowing about: `ToolRegistry#context_for` uses its own `credential_cache` hash, separate from the internal `Server` instance's own cache used by `registry.mcp`'s `tools/call` path — so a credential resolved via both paths for the same tool is cached twice, not shared. Not a correctness issue (both resolve to the same value), just a missed cache-sharing opportunity; `Server` has no way to accept an externally-supplied cache hash today.
