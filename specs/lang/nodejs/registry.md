...
- `nodejs/test/registry.test.js` covers `routes` ordering/shape/collision-warning, `openapi` path-vs-body param placement for both GET and non-GET routes, and `mcp` dispatch (`initialize`, `tools/list`, `tools/call`, `getEnv`, `executeTimeout`).
