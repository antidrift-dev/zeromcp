...
**Shipped**, on branch `registry/csharp`. Implemented exactly as designed below: `Registry.cs` (new `McpHandler` delegate, `RouteDefinition` record, `RegistryOptions`/`ToolRegistry` classes, `Registry.Create` factory), plus the `BuildOpenApiSpec()` visibility change (`private` → `internal`). One addition beyond the literal Public API code block: `RegistryOptions.GetContext` needed an actual wiring point to do anything, since `ZeroMcpServer.CallTool` hardcoded its own `ToolContext` construction with no seam — added an `internal Func<string, Permissions?, ToolContext>? ContextFactory` property on `ZeroMcpServer` (null by default, so `Serve()`/`ServeHttp()` are unaffected), consulted by `CallTool` before falling back to the existing default construction, and set by `Registry.Create` from `options?.GetContext`. Covered by 7 new tests in `RegistryTests.cs` (route filtering, OpenAPI parity, direct tool invocation, `Mcp` dispatch for `tools/list` and `tools/call`, `GetContext` actually taking effect, empty-routes case).

**This is also the first time the whole C# implementation has been build-verified in this effort.** `dotnet` wasn't installed when the cross-language OpenAPI path-param bug fix (see "Fixed" section below) was made — it was reviewed by eye only. `dotnet` (8.0.423) is now installed via the official install script (no cask/sudo needed), and both that earlier fix and this registry compile clean: `dotnet build` on `ZeroMcp.csproj`, `RouteTest.csproj`, `Example.csproj`, and `CredentialTest.csproj` all succeed with 0 errors. `dotnet test` on `ZeroMcp.Tests.csproj` passes 64/67 (the 7 new `RegistryTests` all pass) — the 3 failures are in `TemplateTests.cs` and are pre-existing, confirmed present on `main` before any of this work, unrelated to routes/OpenAPI/registry.
...
**Fixed directly** (not just specified) in `Server.cs`: the `else` branch's `foreach` loop (line 933-937) now `continue`s past any `fieldName` present in `pathParamNames` before adding it to `properties`/`required`, and — when `pathParamNames.Count > 0` (line 949-959) — adds a `parameters` array documenting each path param as `{ name, in: "path", required: true, schema: { type: "string" } }`, matching the `if (routeMethod == "get")` branch's existing path-parameter shape (line 910-928). At the time of this fix, `dotnet` was not available in this environment, so the change was verified by code review only. **Update:** `dotnet` (8.0.423) has since been installed (see Status above) and this fix has now been build-verified for real — `dotnet build` succeeds with 0 errors, and `dotnet test` passes cleanly (no test exercised this exact bug directly, but nothing regressed). This means the C# port's reuse of `BuildOpenApiSpec()` (point 4 above) now gets the corrected, verified behavior for free — no divergent logic for the registry to carry.
